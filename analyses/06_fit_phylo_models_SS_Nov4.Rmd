...
```{r}
ggplot(dat, aes(x = num_temps, y = stderr_ss)) +
...
With less precision (higher standard errors), we would expect a broader distribution of activation energy estimates. Let's look at the distribution of activation energies split by whether standard error was high or low (above or below the median SE). Surprisingly, the estimates with high standard errors do not have a wider distribution, opposite to expectations.

```{r}
dat <- mutate(dat, se_cat = if_else(stderr_ss > median(stderr_ss, na.rm = T), "high standard error", "low standard error"))
```
```{r}
ggplot(dat, aes(x = E_SS)) + 
  geom_histogram() +
...
```{r}
group_by(dat, se_cat)%>%
  summarize(median_AE = median(E_SS))
```
...
ggplot(filter(dat, binomial %in% sp_mm)%>%
       ...
       aes(x = bi_o, y = E_SS, color = same_exp)) +
  geom_point(alpha = 0.5) +
  coord_flip() +
  labs(x = "Species", y = "Activation energy (SS)", color = "Same experiment?")
```
...
We can zoom in on just these cases where there are multiple measurements from the same experiment. Unlike for [Arrhenius](06_fit_phylo_models_Arr_Nov4.md) activation energies, there was not an obvious bias in how minimum vs max development times impacted activation energy estimates. 

```{r}
ggplot(filter(dat, binomial %in% sp_mm, same_exp == 'yes')%>%
         mutate(bi_o = factor(binomial, levels = sp_mm)),
       aes(x = bi_o, y = E_SS, color = trait_detail2)) +
  geom_point(alpha = 1) +
  coord_flip() +
  labs(x = "Species", y = "Activation energy (SS)", color = "Devo metric")
```

This is related to the concerns Peter has raised about *apparent* developmental rates, in that development is not measured for all viable worms (ones that die before maturing are excluded). We can examine whether activation energy varies systematically with developmental metric. It does, in that min devo times tend to yield higher AE estimates.

```{r}
ggplot(dat, aes(y = E_SS, x = trait_detail3)) + 
...
mod0 <- MCMCglmm(E_SS ~ 1, 
                    random = ~ binomial + idh(stderr_ss):units, # w/in spp effx
...
mod1 <- MCMCglmm(E_SS ~ 1, 
                 random = ~ binomial + tree_tips + idh(stderr_ss):units, # w/in spp + phylogenetic effx
...
After fitting, we can again plot the chains. They look good. The within-species effect (`binomial`) is smaller than the phylogenetic effect (`tree_tips`) and it pushes up against zero. 
...
ggplot(mutate(dat, ord_A = factor(Order, levels = ord_A$Order)),
  ...
  geom_boxplot(outlier.color = NA) +
  geom_point(alpha = 0.5, position = position_jitter(width = 0.2), aes(size = 1/stderr_ss)) + 
  coord_flip() +
  theme(axis.title.y = element_blank()) +
...
As seen above, the chosen metric of development can affect activation energy estimates. To check this effect, I added a fixed factor to the model that distinguishes between activation estimates based on mean, min, or unspecific developmental times.

```{r}
mod1.1 <- MCMCglmm(E_SS ~ trait_detail3, 
...
The differences among these groups were rather minimal (non-significant p-values).

```{r}
summary(mod1.1)
```

Thus, it is not clear how valuable this term is in the model. Therefore, I have not included it in further analyses.
...
```{r}
mod3 <- MCMCglmm(E_SS ~ mean_ann_temp_cen, 
                 random = ~ binomial + tree_tips + idh(stderr_ss):units, # w/in spp + phylogenetic effx
                 data = dat, prior = prior2,
                 nitt=63000, thin = 50,
                 ginverse = list(tree_tips=Ainv),
...
ggplot(dat, aes(x = mean_ann_temp, y = E_SS, color = sample_loc_avail)) +
  geom_point(alpha = 0.5, aes(size = 1/stderr_ss)) +
...
A mean temperature does not indicate the temperature range a parasite is exposed to. Thus, as the next modelling step, we'll include location min and max temps. None of these terms are significant and the model DIC decreases, suggesting adding temperature extremes does not improve the model.

```{r}
mod4 <- MCMCglmm(E_SS ~ mean_ann_temp_cen + max_month_temp_cen + min_month_temp_cen, 
                 random = ~ binomial + tree_tips + idh(stderr_ss):units, # w/in spp + phylogenetic effx
                 data = dat, prior = prior2,
                 nitt=63000, thin = 50,
                 ginverse = list(tree_tips=Ainv),
...
```{r}
ggplot(dat, aes(x = max_month_temp, y = E_SS, color = sample_loc_avail)) +
  geom_point(alpha = 0.5, aes(size = 1/stderr_ss )) +
...
```{r}
ggplot(dat, aes(x = min_month_temp, y = E_SS, color = sample_loc_avail)) +
  geom_point(alpha = 0.5, aes(size = 1/stderr_ss )) +
  labs(color = "exact location?", size = "Weight") +
  geom_smooth(method = lm, se = F)
```

This is suggestive that, once controlling for mean temp, activation energies increase with higher max and min temps. However, as neither min nor max seems to explain additional variation in activation energy, we won't consider these temperature extremes further.

```{r}
preds <- predict(mod3) # predicted values with just mean temp
```
```{r}
# ggplot(dat, aes(x = max_month_temp, y = preds - E_SS, color = sample_loc_avail)) +
#   geom_point(alpha = 0.5, aes(size = 1/stderr_ss )) +
#   labs(color = "exact location?", size = "Weight", y = "Residual, mean temp only") +
#   geom_smooth(method = lm, se = F) 
```
```{r}
# ggplot(dat, aes(x = min_month_temp, y = preds - E_SS, color = sample_loc_avail)) +
#   geom_point(alpha = 0.5, aes(size = 1/stderr_ss )) +
#   labs(color = "exact location?", size = "Weight", y = "Residual, mean temp only") +
#   geom_smooth(method = lm, se = F) 
```
...
```{r}
mod5 <- MCMCglmm(E_SS ~ mean_ann_temp_cen + lat_abs, 
                 random = ~ binomial + tree_tips + idh(stderr_ss):units, # w/in spp + phylogenetic effx
                 data = dat, prior = prior2,
                 nitt=63000, thin = 50,
                 ginverse = list(tree_tips=Ainv),
...
```{r}
ggplot(dat, aes(x = lat_abs, y = preds - E_SS, color = sample_loc_avail)) + 
  geom_point(alpha = 0.5, aes(size = 1/stderr_ss )) +
  geom_smooth(method = lm, se = F) +
...
```{r}
mod6 <- MCMCglmm(E_SS ~ mean_ann_temp_cen + dist_zone, 
                 random = ~ binomial + tree_tips + idh(stderr_ss):units, # w/in spp + phylogenetic effx
                 data = dat, prior = prior2,
                 nitt=63000, thin = 50,
                 ginverse = list(tree_tips=Ainv),
...
Several contrasts among these categories are marginally significant and the model DIC decreases.

```{r}
summary(mod6)
```

Consistent with the pattern for temperature and latitude, polar species tend to have higher AE. But weirdly, a non-linear relationship with latitude seems to emerge, with AE peaking in polar habitats, decreasing in temperate habitats, and then increasing again toward the equator.  
...
According to this measure, the model with distribution is better than the one with latitude, which is not surprising given the seemingly non-linear relationship with latitude. Another explanation for this, is that including distribution zone strengthens the estimated effect of mean annual temperature; it explains residual variation around the temp-AE relationship. 
...
```
```{r}
mod7 <- MCMCglmm(E_SS ~ mean_ann_temp_cen + dist_zone + habitat_d1, 
                 random = ~ binomial + tree_tips + idh(stderr_ss):units, # w/in spp + phylogenetic effx
                 data = dat, prior = prior2,
                 nitt=63000, thin = 50,
                 ginverse = list(tree_tips=Ainv),
...
ggplot(filter(dat, !is.na(habitat_d1)),
  ...
  geom_boxplot() +
  geom_point(aes(size = 1/stderr_ss), position = position_jitter(width = 0.2), alpha = 0.3) +
  labs(size = "Weight")
```
...
# ggplot(filter(dat, !is.na(habitat_d1)),
#        aes(x = lat_abs, y = E_SS, color = habitat_d1)) +
#   geom_point(aes(size = 1/stderr_ss), position = position_jitter(width = 0.2), alpha = 0.3) +
...
```{r}
mod8 <- MCMCglmm(E_SS ~ mean_ann_temp + dist_zone * habitat_d1, 
                 random = ~ binomial + tree_tips + idh(stderr_ss):units, # w/in spp + phylogenetic effx
                 data = dat, prior = prior2,
                 nitt=63000, thin = 50,
                 ginverse = list(tree_tips=Ainv),
...
To summarize, AE tends to be higher with low mean locale temperatures and terrestrial habitats.
...
```{r}
mod9 <- MCMCglmm(E_SS ~ mean_ann_temp_cen + dist_zone + habitat_d1 + plant_anim, 
                 random = ~ binomial + tree_tips + idh(stderr_ss):units, # w/in spp + phylogenetic effx
                 data = dat, prior = prior2,
                 nitt=63000, thin = 50,
                 ginverse = list(tree_tips=Ainv),
...
```{r}
mod10 <- MCMCglmm(E_SS ~ mean_ann_temp_cen + dist_zone + habitat_d1 + endo_ecto2, 
                 random = ~ binomial + tree_tips + idh(stderr_ss):units, # w/in spp + phylogenetic effx
                 data = dat, prior = prior2,
                 nitt=63000, thin = 50,
                 ginverse = list(tree_tips=Ainv),
...
The term contrasting animal parasites with and without endotherms in the cycle was significant, and it suggested that species without endotherms have higher AEs.
...
ggplot(dat, aes(x = mean_ann_temp, color = endo_ecto2, y = E_SS)) +
  geom_point(aes(size = 1/stderr_ss), alpha = 0.5) +
  geom_smooth(method = lm, se = F) +
...
```{r}
mod11 <- MCMCglmm(E_SS ~ mean_ann_temp_cen + dist_zone + habitat_d1 + endo_ecto2 + devo_type, 
                 random = ~ binomial + tree_tips + idh(stderr_ss):units, # w/in spp + phylogenetic effx
                 data = dat, prior = prior2,
                 nitt=63000, thin = 50,
                 ginverse = list(tree_tips=Ainv),
...
ggplot(dat, aes(x = devo_type, y = E_SS)) +
  geom_boxplot(outlier.size = NULL, outlier.alpha = 0) +
  geom_point(aes(color = habitat_d1, size = 1/stderr_ss), 
...
Distinguishing between invertebrates and vertebrates as next host slightly improves the model, but the contrasts are not significant.

```{r}
mod12 <- MCMCglmm(E_SS ~ mean_ann_temp_cen + dist_zone + habitat_d1 + endo_ecto2 + devo_type3, 
                 random = ~ binomial + tree_tips + idh(stderr_ss):units, # w/in spp + phylogenetic effx
                 data = dat, prior = prior2,
                 nitt=63000, thin = 50,
                 ginverse = list(tree_tips=Ainv),
...
ggplot(dat, aes(x = devo_type3, y = E_SS, color = devo_type)) +
  geom_boxplot(outlier.size = NULL, outlier.alpha = 0) +
  geom_point(aes(size = 1/stderr_ss), position = position_jitter(width = 0.25, height = 0), alpha = 0.2)+
  coord_flip() +
...
The most complex model, one that includes phylogeny, seemingly important environmental variables, and characteristics of the host-parasite system, explained over 89% of the variation in activation energy. Much of this is due to the random effects (phylogeny and repeated measures account for >70% of the variation), but around 15% of the variation can be explained by fixed effects. 
